...
PageRange = 5                               # number of pages before and after the current page
                                            # to display
#
# show item flags. Set to 1 to enable, set to 0 to disable.
#
showTotalPages = 1                          # show a total number of pages 
showEllipses = 1                            # show a string signifying that not all pages numbers
                                            # are displayed
...
FirstString = "Primeira"                       # content of the "First Page" link
LastString = "Última"                         # content of the "Last Page" link
...
linkString = '<a href="' + linkLocation + "?page=%s&amp;step=" + str(step)

for item, value in extras.items():
  if item == 'txt_assunto':
    value = '%22'.join(value.split('\"'))
    value = '%2B'.join(value.split('+'))
  if item == 'txt_palavra_chave':
    value = '%22'.join(value.split('\"'))
    value = '%2B'.join(value.split('+'))
  if item == 'txt_texto':
    value = '%22'.join(value.split('\"'))
...
if showPrevious:
    if page > 1:
        ...
        returnString = returnString + " " + PreviousString + " "
...
if showNext:
	...
	linkStringTemp = NextString.join(linkStringTemp.split('%s'))
        ...
        returnString = returnString + " " + NextString + " "
...
if showLast:
	...
	linkStringTemp = LastString.join(linkStringTemp.split('%s'))
        ...
        returnString = returnString + " " + LastString + " "
