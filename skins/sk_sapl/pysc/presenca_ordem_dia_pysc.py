...
for i in cod_parlamentar:
    if i not in parlamentares and i not in parlamentares_excl:
        context.zsql.presenca_ordem_dia_incluir_zsql(cod_sessao_plen=cod_sessao_plen,cod_parlamentar=i,dat_ordem=dat_ordem)
    else:
        context.zsql.presenca_ordem_dia_alterar_zsql(cod_sessao_plen=cod_sessao_plen,cod_parlamentar=i,dat_ordem=dat_ordem,ind_excluido=0)

for i in parlamentares:
    if i not in cod_parlamentar:
        context.zsql.presenca_ordem_dia_alterar_zsql(cod_sessao_plen=cod_sessao_plen,cod_parlamentar=i,dat_ordem=dat_ordem,ind_excluido=1)
