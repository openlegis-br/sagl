...
def protocolos(lst_protocolos):
    ...
    for dic in lst_protocolos:
        ...
        if dic['titulo']!=None:
            ...
            tmp_data+='\t\t\t<font color="white"> </font>\n'
            tmp_data+='\t\t</para>\n'
	    tmp_data+='\t\t<barCode code="Code128" x="0.15cm" barHeight="0.34in" barWidth="0.018in">' +dic['codigo']+ '</barCode>\n'
            tmp_data+='\t\t<para style="P2"><b>Protocolo Geral '+dic['titulo']+'/'+dic['ano']+'</b></para>\n'
        if dic['data']!=None:
