...
##parameters=cod_parlamentar, txt_observacao="", vot_parlamentar="", cod_ordem, cod_materia, tip_resultado_votacao
##title=
##
...
if cod_votacao is None:
    context.zsql.votacao_incluir_zsql(num_votos_sim=len(votos_sim),num_votos_nao=len(votos_nao),num_abstencao=len(votos_abstencao),txt_observacao=txt_observacao,cod_ordem=cod_ordem,cod_materia=cod_materia,tip_resultado_votacao=tip_resultado_votacao)
else:
    context.zsql.votacao_atualizar_zsql(cod_votacao=cod_votacao,num_votos_sim=len(votos_sim),num_votos_nao=len(votos_nao),num_abstencao=len(votos_abstencao), txt_observacao=txt_observacao,cod_ordem=cod_ordem,cod_materia=cod_materia,tip_resultado_votacao=tip_resultado_votacao)
