...
class SAPGTool(UniqueObject, SimpleItem, ActionProviderBase):
    ...
    def verifica_esfera_federacao(self):
        ...
        '''
        nome_camara = self.documentos.propriedades.nom_casa
    ...
    def monta_id(self,cod_norma):
        ...
        if consulta:
            ...
            end_web_casa = self.documentos.propriedades.end_web_casa
            sgl_casa = self.documentos.propriedades.sgl_casa.lower()
    ...
    def monta_urn(self, cod_norma):
        ...
        if consulta:
            ...
            localidade = self.zsql.localidade_obter_zsql(cod_localidade = self.documentos.propriedades.cod_localidade)
    ...
    def monta_xml(self,urn,cod_norma):
        ...
        if consulta and publicador:
            ...
            localidade = self.zsql.localidade_obter_zsql(cod_localidade = self.documentos.propriedades.cod_localidade)[0].nom_localidade
            sigla_uf = self.zsql.localidade_obter_zsql(cod_localidade = self.documentos.propriedades.cod_localidade)[0].sgl_uf
            ...
            formato = 'text/html'
            id_documento = u'%s_%s' % (str(cod_norma), self.documentos.norma_juridica.nom_documento)
            if hasattr(self.documentos.norma_juridica,id_documento):
                arquivo = getattr(self.documentos.norma_juridica,id_documento)
    ...
    def get_brasao(self):
        ...
        id_logo = portal.documentos.propriedades.id_logo
        url = self.url() + '/documentos/propriedades/logo_casa.gif'
    ...
    def ata_gerar_odt(self, inf_basicas_dic, lst_mesa, lst_presenca_sessao, lst_materia_apresentada, lst_reqplen, lst_reqpres, lst_indicacao, lst_presenca_ordem_dia, lst_votacao, lst_presenca_expediente, lst_oradores, lst_presenca_encerramento, lst_presidente, lst_psecretario, lst_ssecretario, nom_arquivo):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/ata.odt"
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.ata_sessao.manage_addFile(id=output_file_odt,file=data)
    ...
    def ata_gerar_pdf(self, cod_sessao_plen):
    	...
    	url = self.documentos.ata_sessao.absolute_url() + "/%s"%nom_arquivo_odt
    	...
    	for file in [output_file_pdf]:
    	    os.unlink(file)
    	    self.documentos.ata_sessao.manage_addFile(id=nom_arquivo_pdf,file=data)
    ...
    def iom_gerar_odt(self, inf_basicas_dic, lst_mesa, lst_presenca_sessao, lst_materia_apresentada, lst_reqplen, lst_reqpres, lst_indicacao, lst_presenca_ordem_dia, lst_votacao, lst_presenca_expediente, lst_oradores, lst_presenca_encerramento, lst_presidente, lst_psecretario, lst_ssecretario):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/iom.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
    ...
    def materia_apreciada_gerar_odt(self, inf_basicas_dic, lst_votacao):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/materia_apreciada.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
    ...
    def materia_apresentada_gerar_odt(self, inf_basicas_dic, lst_materia_apresentada):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/materia_apresentada.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
    ...
    def ordem_dia_gerar_odt(self, inf_basicas_dic, lst_pdiscussao, lst_sdiscussao, lst_discussao_unica, lst_presidente, nom_arquivo):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/ordem_dia.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.pauta_sessao.manage_addFile(id=output_file_odt,file=data)
    ...
    def ordem_dia_gerar_pdf(self, cod_sessao_plen):
    	...
    	url = self.documentos.pauta_sessao.absolute_url() + "/%s"%nom_arquivo_odt
    	odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
    	output_file_pdf = os.path.normpath(nom_arquivo_pdf)
    	renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
    	renderer.run()
    	...
    	for file in [output_file_pdf]:
    	    os.unlink(file)
    	    self.documentos.pauta_sessao.manage_addFile(id=nom_arquivo_pdf,file=data)

    def oradores_gerar_odt(self, inf_basicas_dic, lst_oradores, lst_presidente, nom_arquivo):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/oradores.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.oradores_expediente.manage_addFile(id=output_file_odt,file=data)
    ...
    def oradores_gerar_pdf(self,cod_sessao_plen):
        ...
        url = self.documentos.oradores_expediente.absolute_url() + "/%s"%nom_arquivo_odt
        odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
        output_file_pdf = os.path.normpath(nom_arquivo_pdf)
        renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
        renderer.run()
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.oradores_expediente.manage_addFile(id=nom_arquivo_pdf,file=data)
    ...
    def expediente_gerar_odt(self, inf_basicas_dic, lst_indicacoes, lst_requerimentos, lst_mocoes, lst_oradores, lst_presidente, nom_arquivo):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/expediente.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
    ...
    def resumo_gerar_odt(self, inf_basicas_dic, lst_mesa, lst_presenca_sessao, lst_materia_apresentada, lst_reqplen, lst_reqpres, lst_indicacao, lst_presenca_ordem_dia, lst_votacao, lst_presenca_expediente, lst_oradores, lst_presenca_encerramento, lst_presidente, lst_psecretario, lst_ssecretario):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/resumo.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
    ...
    def doc_acessorio_gerar_odt(self, inf_basicas_dic, nom_arquivo, des_tipo_documento, nom_documento, txt_ementa, dat_documento, data_documento, nom_autor, materia_vinculada, modelo_proposicao):
        url = self.documentos.modelo.materia.documento_acessorio.absolute_url() + "/%s" % modelo_proposicao
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.materia_odt.manage_addFile(id=nom_arquivo,file=data)

    def doc_acessorio_gerar_pdf(self, cod_documento):
        nom_arquivo_odt = "%s"%cod_documento+'.odt'
        nom_arquivo_pdf = "%s"%cod_documento+'.pdf'
        url = self.documentos.materia_odt.absolute_url() + "/%s"%nom_arquivo_odt
        odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
        output_file_pdf = os.path.normpath(nom_arquivo_pdf)
        renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
        renderer.run()
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.materia.manage_addFile(id=nom_arquivo_pdf,file=data)

    def oficio_ind_gerar_odt(self, inf_basicas_dic, lst_indicacao, lst_presidente):
        url = self.documentos.modelo.sessao_plenaria.absolute_url() + "/oficio_indicacao.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
    ...
    def emenda_gerar_odt(self, inf_basicas_dic, num_proposicao, nom_arquivo, des_tipo_materia, num_ident_basica, ano_ident_basica, txt_ementa, materia_vinculada, dat_apresentacao, nom_autor, apelido_autor, modelo_proposicao):
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.emenda.manage_addFile(id=nom_arquivo,file=data)

    def emenda_gerar_pdf(self,cod_emenda):
        nom_arquivo_odt = "%s"%cod_emenda+'_emenda.odt'
        nom_arquivo_pdf = "%s"%cod_emenda+'_emenda.pdf'
        url = self.documentos.emenda.absolute_url() + "/%s"%nom_arquivo_odt
        odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
        output_file_pdf = os.path.normpath(nom_arquivo_pdf)
        renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
        renderer.run()
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.emenda.manage_addFile(id=nom_arquivo_pdf,file=data)
    ...
    def materia_gerar_pdf(self, cod_materia):
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.materia.manage_addFile(id=nom_arquivo_pdf1, file=data)
    ...
    def norma_gerar_odt(self, inf_basicas_dic, nom_arquivo, des_tipo_norma, num_norma, ano_norma, dat_norma, data_norma, txt_ementa, modelo_norma):
        url = self.documentos.modelo.norma.absolute_url() + "/%s" % modelo_norma
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.norma_juridica.manage_addFile(id=nom_arquivo,file=data)
    ...
    def norma_gerar_pdf(self, cod_norma):
        ...
        url = self.documentos.norma_juridica.absolute_url() + "/%s"%nom_arquivo_odt
        odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
        output_file_pdf = os.path.normpath(nom_arquivo_pdf1)
        renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
        renderer.run()
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.norma_juridica.manage_addFile(id=nom_arquivo_pdf1, file=data)
    ...
    def oficio_gerar_odt(self, inf_basicas_dic, nom_arquivo, sgl_tipo_documento, num_documento, ano_documento, txt_ementa, dat_documento, dia_documento, nom_autor, modelo_documento):
        url = self.documentos.modelo.documento_administrativo.absolute_url() + "/%s" % modelo_documento
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.administrativo.manage_addFile(id=nom_arquivo,file=data)
    ...
    def oficio_gerar_pdf(self, cod_documento):
        ...
        url = self.documentos.administrativo.absolute_url() + "/%s"%nom_arquivo_odt
        odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
        output_file_pdf = os.path.normpath(nom_arquivo_pdf1)
        renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
        renderer.run()
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.administrativo.manage_addFile(id=nom_arquivo_pdf1, file=data)
    ...
    def parecer_gerar_odt(self, inf_basicas_dic, nom_arquivo, nom_comissao, materia, nom_autor, txt_ementa, tip_apresentacao, tip_conclusao, data_parecer, nom_relator, lst_composicao):
        url = self.documentos.modelo.materia.parecer.absolute_url() + "/parecer.odt"
        template_file = cStringIO.StringIO(urllib.urlopen(url).read())
        brasao_file = self.get_brasao()
        # atribui o brasao no locals
        exec 'brasao = brasao_file'
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.parecer_comissao.manage_addFile(id=nom_arquivo,file=data)
    ...
    def parecer_gerar_pdf(self, cod_parecer):
        ...
        url = self.documentos.parecer_comissao.absolute_url() + "/%s"%nom_arquivo_odt
        odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
        output_file_pdf = os.path.normpath(nom_arquivo_pdf1)
        renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
        renderer.run()
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.parecer_comissao.manage_addFile(id=nom_arquivo_pdf1, file=data)
    ...
    def substitutivo_gerar_odt(self, inf_basicas_dic, num_proposicao, nom_arquivo, des_tipo_materia, num_ident_basica, ano_ident_basica, txt_ementa, materia_vinculada, dat_apresentacao, nom_autor, apelido_autor, modelo_proposicao):
        ...
        for file in [output_file_odt]:
            os.unlink(file)
            self.documentos.substitutivo.manage_addFile(id=nom_arquivo,file=data)
    ...
    def substitutivo_gerar_pdf(self,cod_substitutivo):
        ...
        url = self.documentos.substitutivo.absolute_url() + "/%s"%nom_arquivo_odt
        odtFile = cStringIO.StringIO(urllib.urlopen(url).read())
        output_file_pdf = os.path.normpath(nom_arquivo_pdf)
        renderer = Renderer(odtFile,locals(),output_file_pdf,pythonWithUnoPath='/usr/bin/python3',forceOoCall=True)
        renderer.run()
        ...
        for file in [output_file_pdf]:
            os.unlink(file)
            self.documentos.substitutivo.manage_addFile(id=nom_arquivo_pdf,file=data)

    def resize_and_crop(self,cod_parlamentar):
        utool = getToolByName(self, 'portal_url')
        portal = utool.getPortalObject()
        image_file = '%s' % (cod_parlamentar) + "_foto_parlamentar"
        url = self.url() + '/documentos/parlamentar/fotos/' + image_file
        opener = urllib.urlopen(url)
        ...
        foto = getattr(self.documentos.parlamentar.fotos,image_file) 
    ...
    def protocolo_barcode(self,cod_protocolo):
        sgl_casa = self.documentos.propriedades.sgl_casa
        for protocolo in self.zsql.protocolo_obter_zsql(cod_protocolo=cod_protocolo):
          ...
          pdf_protocolo = self.documentos.protocolo.absolute_url() + "/" +  str(cod_protocolo) + "_protocolo.pdf"
        ...
        utool = getToolByName(self, 'portal_url')
        portal = utool.getPortalObject()
        url = self.url() + '/documentos/protocolo/' + pdf_file
        opener = urllib.urlopen(url)
        ...
        for item in [outputStream]:
          if nom_pdf_protocolo in self.documentos.protocolo:
            documento = getattr(self.documentos.protocolo,nom_pdf_protocolo)
    ...
    def processo_eletronico_gerar_pdf(self,cod_materia):
        ...
        if proposicao:
           ...
           pdf_proposicao = self.documentos.proposicao.absolute_url() + "/" +  str(cod_proposicao) + "_signed.pdf"
           texto_materia = cStringIO.StringIO(urllib.urlopen(pdf_proposicao).read())
           merger.append(texto_materia)
        else:
           if hasattr(self.documentos.materia, str(cod_materia) + '_texto_integral.pdf'):
              pdf_materia = self.documentos.materia.absolute_url()+ "/" + cod_materia + "_texto_integral.pdf"
        ...
        for anexada in self.zsql.anexada_obter_zsql(cod_materia_principal=cod_materia,ind_excluido=0):
           dic_mat = {}
           if hasattr(self.documentos.materia, str(anexada.cod_materia_anexada) + '_texto_integral.pdf'):
              dic_mat['pdf_anexada'] = self.documentos.materia.absolute_url()+ "/" + str(anexada.cod_materia_anexada) + "_texto_integral.pdf"
        ...
        for emenda in self.zsql.emenda_obter_zsql(cod_materia=cod_materia,ind_excluido=0):
           dic_emendas = {}
           if hasattr(self.documentos.emenda, str(emenda.cod_emenda) + '_emenda.pdf'):
              dic_emendas['pdf_emenda'] = self.documentos.emenda.absolute_url()+ "/" + str(emenda.cod_emenda) + "_emenda.pdf"
        ...
        for substitutivo in self.zsql.substitutivo_obter_zsql(cod_materia=cod_materia):
           dic_substitutivos = {}
           if hasattr(self.documentos.substitutivo, str(substitutivo.cod_substitutivo) + '_substitutivo.pdf'):
              dic_substitutivos['pdf_substitutivo'] = self.documentos.substitutivo.absolute_url()+ "/" + str(substitutivo.cod_substitutivo) + "_substitutivo.pdf"
        ...
        #   dic_comissao={}
        #   if hasattr(self.documentos.parecer_comissao, str(relatoria.cod_relatoria) + '_parecer.pdf'):
        #      dic_comissao['pdf_relatoria'] = self.documentos.parecer_comissao.absolute_url()+ "/" + relatoria.cod_relatoria + "_parecer.pdf"
        ...
        for documento in self.zsql.documento_acessorio_obter_zsql(cod_materia = cod_materia,ind_excluido=0):
           ...
           if proposicao:
              ...
              dic_dados={}
              if hasattr(self.documentos.materia, str(documento.cod_documento) + '.pdf'):
                 dic_dados['pdf_documento'] = self.documentos.materia.absolute_url()+ "/" + documento.cod_documento + ".pdf"
        ...
        for tramitacao in self.zsql.tramitacao_obter_zsql(cod_materia=cod_materia,ind_excluido=0):
           dic_tramitacoes={}
           if hasattr(self.documentos.materia.tramitacao, str(tramitacao.cod_tramitacao) + '_tram_signed.pdf'):
              dic_tramitacoes['pdf_tramitacao'] = self.documentos.materia.tramitacao.absolute_url()+ "/" + tramitacao.cod_tramitacao + "_tram_signed.pdf"
           #   lst_tramitacoes.append(dic_tramitacoes)
           elif hasattr(self.documentos.materia.tramitacao, str(tramitacao.cod_tramitacao) + '_tram.pdf'):
              dic_tramitacoes['pdf_tramitacao'] = self.documentos.materia.tramitacao.absolute_url()+ "/" + tramitacao.cod_tramitacao + "_tram.pdf"
              lst_tramitacoes.append(dic_tramitacoes)
           #if hasattr(self.documentos.materia.tramitacao, str(tramitacao.cod_tramitacao) + '_tram_signed.pdf') or hasattr(self.documentos.materia.tramitacao, str(tramitacao.cod_tramitacao) + '_tram.pdf'):
    ...
    def proposicao_autuar(self,cod_proposicao):
        ...
        pdf_proposicao = self.documentos.proposicao.absolute_url() + "/" +  nom_pdf_proposicao
        nom_pdf_proposicao = str(cod_proposicao) + "_signed.pdf"
        ...
        utool = getToolByName(self, 'portal_url')
        portal = utool.getPortalObject()
        url = self.url() + '/documentos/proposicao/' + nom_pdf_proposicao
        opener = urllib.urlopen(url)
        ...
        for item in [outputStream]:
          if nom_pdf_saida in self.documentos.materia:
            documento = getattr(self.documentos.materia,nom_pdf_saida)
            documento.manage_upload(file=data)
          else:
            self.documentos.materia.manage_addFile(id=nom_pdf_saida,file=data)
    ...
    def restpki_client(self):
        ...
        restpki_access_token = self.documentos.propriedades.restpki_access_token            
    ...
    def pades_signature(self, cod_proposicao):
        ...
        utool = getToolByName(self, 'portal_url')
        portal = utool.getPortalObject()
        url = self.url() + '/documentos/proposicao/' + pdf_file
        opener = urllib.urlopen(url)
        f = open('/tmp/' + pdf_file, 'wb').write(opener.read())
        tmp_path = '/tmp'
        pdf_tmp = pdf_file
    ...
    def pades_cosignature(self, cod_proposicao):
        ...
        utool = getToolByName(self, 'portal_url')
        portal = utool.getPortalObject()
        url = self.url() + '/documentos/proposicao/' + pdf_file_signed
        opener = urllib.urlopen(url)
    ...
    def pades_signature_action(self, token, cod_proposicao):
        ...
        for file in [filename]:
            self.documentos.proposicao.manage_delObjects(ids=old_filename)
            os.unlink(file)
            if filename in self.documentos.proposicao:
              documento = getattr(self.documentos.proposicao,filename)
              documento.manage_upload(file=data)
            else:
              self.documentos.proposicao.manage_addFile(id=filename,file=data)
    ...
    def tramitacao_signature(self, cod_tramitacao):
        ...
        utool = getToolByName(self, 'portal_url')
        portal = utool.getPortalObject()
        url = self.url() + '/documentos/materia/tramitacao/' + pdf_file
        opener = urllib.urlopen(url)
        f = open('/tmp/' + pdf_file, 'wb').write(opener.read())
        tmp_path = '/tmp'
        pdf_tmp = pdf_file
    ...
    def tramitacao_signature_action(self, token, cod_tramitacao):
        ...
        for file in [filename]:
            self.documentos.materia.tramitacao.manage_delObjects(ids=old_filename)
            os.unlink(file)
            if filename in self.documentos.materia.tramitacao:
              documento = getattr(self.documentos.materia.tramitacao,filename)
              documento.manage_upload(file=data)
            else:
              self.documentos.materia.tramitacao.manage_addFile(id=filename,file=data)
