...
for norma in context.zsql.norma_juridica_obter_zsql(cod_norma=cod_norma):
  if texto_original:
    if hasattr(context.sagl_documentos.norma_juridica, "%s"%cod_norma+'_texto_integral_signed.pdf'):
    ...
    elif hasattr(context.sagl_documentos.norma_juridica, "%s"%cod_norma+'_texto_integral.pdf'):
  ...
  arquivo = getattr(context.sagl_documentos.norma_juridica,id_documento) 
