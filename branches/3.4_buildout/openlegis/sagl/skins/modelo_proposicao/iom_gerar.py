...
for sessao in context.zsql.sessao_plenaria_obter_zsql(cod_sessao_plen=cod_sessao_plen, ind_excluido=0):
    ...
    for materia in context.zsql.materia_obter_zsql(cod_materia=item.cod_materia,tip_id_basica=9,ind_excluido=0):
      ...
      if len(autoria) > 0:
          ...
          if autor.des_tipo_autor == "Parlamentar":
              ...
              dic_reqpl["nom_autor"] = parlamentar.nom_completo.encode('utf-8')
          elif autor.des_tipo_autor == "Comissao":
              comissao = context.zsql.comissao_obter_zsql(cod_comissao=autor.cod_comissao)[0]
              dic_reqpl["nom_autor"] = comissao.nom_comissao.encode('utf-8')
          elif autor.des_tipo_autor == "Bancada":
              bancada = context.zsql.bancada_obter_zsql(cod_bancada=autor.cod_bancada)[0]
              dic_reqpl["nom_autor"] = bancada.nom_bancada.encode('utf-8')
          else:
              dic_reqpl["nom_autor"] = autor.nom_autor.upper().encode('utf-8')
  ...
  for item in context.zsql.expediente_materia_obter_zsql(cod_sessao_plen=cod_sessao_plen,ind_excluido=0):
    for materia in context.zsql.materia_obter_zsql(cod_materia=item.cod_materia,tip_id_basica=3,ind_excluido=0):
      ...
      if len(autoria) > 0:
          ...
          if autor.des_tipo_autor == "Parlamentar":
              parlamentar = context.zsql.parlamentar_obter_zsql(cod_parlamentar=autor.cod_parlamentar)[0]     
              dic_reqpr["nom_autor"] = parlamentar.nom_completo.encode('utf-8')
          elif autor.des_tipo_autor == "Comissao":
              comissao = context.zsql.comissao_obter_zsql(cod_comissao=autor.cod_comissao)[0]
              dic_reqpr["nom_autor"] = comissao.nom_comissao.encode('utf-8')
          elif autor.des_tipo_autor == "Bancada":
              bancada = context.zsql.bancada_obter_zsql(cod_bancada=autor.cod_bancada)[0]
              dic_reqpr["nom_autor"] = bancada.nom_bancada.encode('utf-8')
          else:
              dic_reqpr["nom_autor"] = autor.nom_autor.upper().encode('utf-8')
  ...
  for item in context.zsql.expediente_materia_obter_zsql(cod_sessao_plen=cod_sessao_plen,ind_excluido=0):
    for materia in context.zsql.materia_obter_zsql(cod_materia=item.cod_materia,tip_id_basica=8,ind_excluido=0):
      ...
      if len(autoria) > 0:
          ...
          if autor.des_tipo_autor == "Parlamentar":
              parlamentar = context.zsql.parlamentar_obter_zsql(cod_parlamentar=autor.cod_parlamentar)[0]     
              dic_ind["nom_autor"] = parlamentar.nom_completo.encode('utf-8')
          elif autor.des_tipo_autor == "Comissao":
              comissao = context.zsql.comissao_obter_zsql(cod_comissao=autor.cod_comissao)[0]
              dic_ind["nom_autor"] = comissao.nom_comissao.encode('utf-8')
          elif autor.des_tipo_autor == "Bancada":
              bancada = context.zsql.bancada_obter_zsql(cod_bancada=autor.cod_bancada)[0]
              dic_ind["nom_autor"] = bancada.nom_bancada.encode('utf-8')
          else:
              dic_ind["nom_autor"] = autor.nom_autor.upper().encode('utf-8')
  ...
  for votacao in context.zsql.votacao_ordem_dia_obter_zsql(cod_sessao_plen=cod_sessao_plen, ind_excluido=0):
     # Seleciona os detalhes de uma mat�ria
      ...
      if len(autoria) > 0: # se existe autor
          ...
          if autor.des_tipo_autor == "Parlamentar":
              ...
              dic_votacao["nom_autor"] = autor.nom_autor.upper().encode('utf-8') 
  ...
  # Lista presen�a no Grande Expediente
...
return context.iom_gerar_odt(inf_basicas_dic, lst_mesa, lst_presenca_sessao, lst_materia_apresentada, lst_reqplen, lst_reqpres, lst_indicacao, lst_presenca_ordem_dia, lst_votacao, lst_presenca_expediente, lst_oradores, lst_presenca_encerramento, lst_presidente, lst_psecretario, lst_ssecretario)
