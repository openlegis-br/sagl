...
manage_addSAGLOAIServerForm = PageTemplateFile('www/addSAGLOAIServer', globals())
manage_addSAGLOAIServerForm.__name__='addSAGLOAIServer'

def manage_addSAGLOAIServer(self, email, titulo, batch_size, base_url, id='oai', REQUEST=None):
    """Adicionar um servidor OAI para integracao com o LeXML.
    """
    SAGL_OAI = SAGLOAIServer(email,titulo,batch_size,base_url)
    self._setObject(id, SAGL_OAI)
    if REQUEST is None:
        return
    try:
...
class SAGLOAIServer(SimpleItem.SimpleItem,Persistent):
    ...
    manage_editForm = PageTemplateFile(
        "www/editSAGLOAIServer", globals(), __name__='manage_editForm')
