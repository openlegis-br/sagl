...
INSTALL_HOME=/var/interlegis/SAPL-2.6
EFFECTIVE_USER=zope
...
SAPL_HOME=$INSTANCE_HOME/sapl26
...
#####
# Verifica se existe uma instalacao do SAPL 2.5
if [ -d /var/interlegis/SAPL-2.5/ ]; then
    INST_PATH="/var/interlegis/SAPL-2.5"
else
    INST_PATH="/var/interlegis/SAPL-2.5"
fi
...
if [ -e $INST_PATH ]; then
    echo ""
    echo -e "\033[1;31mUma instalação do SAPL 2.5 foi detectada\033[m"
    echo -e "\033[1;31mRemova a versão instalada e execute o procedimento descrito em:\033[m"
    echo -e "\033[1;31mhttp://openlegis.googlecode.com/wiki/InstalacaoOpenLegis26\033[m"
...
then
    echo ""
    echo -e "\033[1;31mUma instalação do SAPL foi detectada em $INSTALL_HOME.\033[m"
    echo -e "\033[1;31mInstalação abortada\033[m"
    echo ""
    exit 1
fi
...
if [ "$?" = "0" ]; then
    ...
    echo ""
    echo -e "\033[1;31mUma instalação do SAPL foi detectada\033[m"
...
if [ ! -e $PACKAGES_DIR ]
then
    echo "Nota: Este script precisa ser executado dentro de um diretório.  Uso: sudo ./install.sh  (or su ; ./install.sh)"
    exit 1
fi
...
echo ""
echo -e "\033[32mInstalando SAPL 2.6 em $INSTALL_HOME\033[m"
sleep 3
echo ""
...
echo "<zodb_db documentos>
    # Zodb para conter documentos do sapl
    <filestorage>
...
if [ -d $INSTALL_HOME ]
    then
    mkdir $RECEIPTS_HOME
    echo "Instalação do OpenLegis 2.6 finalizada em" `date` > $RECEIPTS_HOME/installReceipt.txt
    echo " "
    echo "#####################################################################"
    ...
    echo " "
    echo "OpenLegis foi instalado com sucesso em $INSTALL_HOME"
